...
export const drops = (svg, scales, configuration) => data => {
    const dropsContainer = svg.append('g').classed('drops-container', true);

    const dropLines = dropsContainer.selectAll('.drops').data(data);
    ...
    dropLines[0].forEach(line => {
        const drops = d3.select(line).selectAll('.drop').data(d => filterData(d.dates, scales.x));
